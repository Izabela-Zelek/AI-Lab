...
void Environment::doTimestep(Grid& grid)
{
	...
	bool obstacle = false;
	float reward = 0.0f;
	if (currentAgentCoords.x < 19)
	{
		if (grid.nodes[currentAgentCoords.x][currentAgentCoords.y - 1].containsObstacle || grid.nodes[currentAgentCoords.x - 1][currentAgentCoords.y - 1].containsObstacle)
		{
			reward = calculateReward(previousAgentCoords, selectedAction, currentAgentCoords,true);
		}
		else
		{
			reward = calculateReward(previousAgentCoords, selectedAction, currentAgentCoords,false);
		}
	}
	else
	{
...
float Environment::calculateReward(sf::Vector2i previousAgentCoords, int selectedAction, sf::Vector2i currentAgentCoords,bool obstacle)
{
	float reward = 0.0f;
	// check if the goal state has been reached
	if (currentAgentCoords.x == goalLocationXY.x && currentAgentCoords.y == goalLocationXY.y && !obstacle)
	{
		reward = goalReward;
		goalReached = true;
	}
